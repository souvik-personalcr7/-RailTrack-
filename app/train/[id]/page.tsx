...
import { ArrowLeft, Share2, Check, MapPin, CloudSun, Mountain, AlertCircle, RefreshCw } from 'lucide-react';
...
export default function TrainJourneyPage({ params }: { params: { id: string } }) {
  ...
  const handleShare = () => {
    ...
    if (typeof navigator.share === 'function') {
      navigator
        .share({ title: `RailTrack – ${journey?.name || `Train #${trainId}`}`, url: shareUrl })
        .catch(() => {
  ...
  return (
    <div className="space-y-4 pt-0 sm:pt-2 pb-2">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-slate-200/60 dark:bg-slate-800/60 border border-slate-300/40 dark:border-slate-700/40 px-3.5 py-2 text-xs font-semibold text-slate-800 dark:text-slate-200 hover:bg-slate-300/70 dark:hover:bg-slate-700/70 transition-all disabled:opacity-60 disabled:cursor-not-allowed active:scale-95 shadow-sm"
      ...
      <MobileJourneySummary journey={journey} onRefresh={() => !isRefetching && refetch()} isRefreshing={isRefetching} />

      {/* ─── Hero Journey Card (desktop) ─── */}
      <div className="hidden md:block">
        <JourneyCard journey={journey} onRefresh={() => !isRefetching && refetch()} isRefreshing={isRefetching} />
      </div>
