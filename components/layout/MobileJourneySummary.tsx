...
import { Activity, Clock, Gauge, RefreshCw } from 'lucide-react';
...
export function MobileJourneySummary({ journey, onRefresh, isRefreshing }: MobileJourneySummaryProps) {
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {onRefresh && (
            <button
              ...
              className="flex h-7 w-7 items-center justify-center rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              title="Refresh Live Data"
            >
              <RefreshCw className={cn('h-3.5 w-3.5', isRefreshing && 'animate-spin text-rail-blue')} />
            </button>
          )}
          <span
            className={cn(
